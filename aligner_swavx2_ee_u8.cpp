 ...
 * paper and implementation by Michael Farrar.
 * But extended for AVX2 256-bit vector size.
 * 
 * See:
 ...
 *
 * Given the drawbacks, it is tempting to use AVX2 dynamic programming as a
 ...
 */
void SwAligner::buildQueryProfileEnd2EndAvx2U8(bool fw) {
	bool& done = fw ? avx2U8fwBuilt_ : avx2U8rcBuilt_;
	if(done) {
		return;
	}
	done = true;
	...
	size_t n128s =
		...
		* 2;                    // & gap barrier data
	assert_gt(n128s, 0);
	AVX2Data& d = fw ? avx2U8fw_ : avx2U8rc_;
...
/**
 * Aligns by filling a dynamic programming matrix with the AVX2-accelerated,
...
TAlScore SwAligner::alignGatherEE8(int& flag, bool debug) {
	...
	AVX2Data& d = fw_ ? avx2U8fw_ : avx2U8rc_;
	AVX2Metrics& met = extend_ ? avx2U8ExtendMet_ : avx2U8MateMet_;
	if(!debug) met.dp++;
	buildQueryProfileEnd2EndAvx2U8(fw_);
	assert(!d.profbuf_.empty());
...
/**
 * Solve the current alignment problem using AVX2 instructions that operate on 32
 * unsigned 8-bit values packed into a single 256-bit register.
 */
TAlScore SwAligner::alignNucleotidesEnd2EndAvx2U8(int& flag, bool debug) {
	assert_leq(rdf_, rd_->length());
	assert_leq(rdf_, qu_->length());
	assert_lt(rfi_, rff_);
	assert_lt(rdi_, rdf_);
 ...
 * 
 */
bool SwAligner::gatherCellsNucleotidesEnd2EndAvx2U8(TAlScore best) {
 ...
 */
bool SwAligner::backtraceNucleotidesEnd2EndAvx2U8(
	...
	RandomSource&  rnd)    // random gen, to choose among equal paths
{
	...
	size_t trimBeg = 0;
	size_t ct = AVX2Matrix::H; // cell type
	// Row and col in terms of where they fall in the AVX2 vector matrix
	...
	while((int)row >= 0) {
		...
		if(reportedThru) {
			...
			if(row > 0) {
				...
				if(ct == AVX2Matrix::E) { // AKA rdgap
					assert_gt(col, 0);
					TAlScore sc_cur = ((TCScore*)(cur_vec + AVX2Matrix::E))[rowelt] + offsetsc;
					assert(gapsAllowed);
					...
					TAlScore sc_h_left = ((TCScore*)(left_vec + AVX2Matrix::H))[left_rowelt] + offsetsc;
					if(sc_h_left > floorsc && sc_h_left - sc_->readGapOpen() == sc_cur) {
						mask |= (1 << 0);
					}
					// Get E score of cell to the left
					TAlScore sc_e_left = ((TCScore*)(left_vec + AVX2Matrix::E))[left_rowelt] + offsetsc;
				...
				} else if(ct == AVX2Matrix::F) { // AKA rfgap
					assert_gt(row, 0);
					assert(gapsAllowed);
					TAlScore sc_h_up = ((TCScore*)(up_vec  + AVX2Matrix::H))[up_rowelt] + offsetsc;
					TAlScore sc_f_up = ((TCScore*)(up_vec  + AVX2Matrix::F))[up_rowelt] + offsetsc;
					TAlScore sc_cur  = ((TCScore*)(cur_vec + AVX2Matrix::F))[rowelt] + offsetsc;
					...
					assert_eq(AVX2Matrix::H, ct);
					TAlScore sc_cur      = ((TCScore*)(cur_vec + AVX2Matrix::H))[rowelt]    + offsetsc;
					TAlScore sc_f_up     = ((TCScore*)(up_vec  + AVX2Matrix::F))[up_rowelt] + offsetsc;
					TAlScore sc_h_up     = ((TCScore*)(up_vec  + AVX2Matrix::H))[up_rowelt] + offsetsc;
					TAlScore sc_h_left   = col > 0 ? (((TCScore*)(left_vec   + AVX2Matrix::H))[left_rowelt]   + offsetsc) : floorsc;
					TAlScore sc_e_left   = col > 0 ? (((TCScore*)(left_vec   + AVX2Matrix::E))[left_rowelt]   + offsetsc) : floorsc;
					TAlScore sc_h_upleft = col > 0 ? (((TCScore*)(upleft_vec + AVX2Matrix::H))[upleft_rowelt] + offsetsc) : floorsc;
				...
				assert(!empty || !canMoveThru || ct == AVX2Matrix::H);
			}
		}
