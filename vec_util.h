 ...
 * Generalized by Igor Sfiligoi <isfiligoi@sdsc.edu>
 *
...
public:
	...
	static constexpr size_t AlignBytes = VecC::AlignBytes;

	/**
	 * Allocate initial default of S elements.
	 */
	explicit EList_vec(int cat = 0) :
	...
	void zero() {
		if(cur_ > 0) {
			memset(list_, 0, cur_ * sizeof(VecT));
		}
	}
	 ...
	 * Return a reference to the ith element.
	 */
	inline VecT operator[](size_t i) const {
		assert_lt(i, cur_);
		return list_[i];
	}
	 ...
	 * Return a reference to the ith element.
	 */
	inline VecT get(size_t i) const {
		return operator[](i);
	}
	 ...
	 */
	VecT *ptr() { return list_; }

	/**
	 * Return a const pointer to the beginning of the buffer.
	 */
	const VecT *ptr() const { return list_; }
...
private:
	 ...
	 */
	VecT *alloc(size_t sz) {
		VecT* last_alloc_;
		try {
			last_alloc_ = new VecT[sz + 2];
		} catch(std::bad_alloc& e) {
			std::cerr << "Error: Out of memory allocating " << sz << " VecT's for DP matrix: '" << e.what() << "'" << std::endl;
			throw e;
		}
                this->last_alloc_ = last_alloc_;
		VecT* tmp = last_alloc_;
		size_t tmpint = (size_t)tmp;
		constexpr size_t align_mask = AlignBytes-1;
		// Align it!
		if((tmpint & align_mask) != 0) {
			tmpint += align_mask;
			tmpint &= (~align_mask);
			tmp = reinterpret_cast<VecT*>(tmpint);
		}
		assert_eq(0, (tmpint & align_mask)); // should be aligned to AlignBytes
		assert(tmp != NULL);
#ifdef USE_MEM_TALLY
	...
	void expandCopyExact(size_t newsz) {
		if(newsz <= sz_) return;
                VecT* prev_last_alloc = last_alloc_;
		VecT* tmp = alloc(newsz);
		assert(tmp != NULL);
		size_t cur = cur_;
		if(list_ != NULL) {
 			for(size_t i = 0; i < cur_; i++) {
				...
				tmp[i] = list_[i];
                        ...
                        VecT* current_last_alloc = last_alloc_;
	...
	VecT*    last_alloc_; // what new[] originally returns
	VecT    *list_;       // list ptr, aligned version of what new[] returns
	size_t   sz_;         // capacity
 ...
 */
template<class VecC>
class VECCheckpointer {

public:
	typedef typename VecC::VecT VecT;
	static constexpr size_t U8Num = VecC::U8Num;

	VECCheckpointer() { reset(); }
...
//		debug_ = debug;
		...
		commitMap_.clear();
		firstCommit_ = true;
		size_t perword = (is8 ? U8Num : (U8Num/2));
		is8_ = is8;
	...
	int64_t debugCell(size_t row, size_t col, int hef) const {
		assert(debug_);
		const VecT* ptr = qcolsD_.ptr() + hef;
		// Fast forward to appropriate column
		ptr += ((col * niter_) << 2);
		size_t mod = row % niter_; // which VecT
		size_t div = row / niter_; // offset into VecT
	 ...
	 */
	void commitCol(VecT *pvH, VecT *pvE, VecT *pvF, size_t coli);
	...
	size_t        niter_;   // # VecT words per column
	EList_vec<VecC> qcols_; // checkpoint E/F/H values for select columns
	
	bool          debug_;   // get debug checkpoints? (i.e. fill qcolsD_?)
	EList_vec<VecC> qcolsD_;// checkpoint E/F/H values for all columns (debug)
};
