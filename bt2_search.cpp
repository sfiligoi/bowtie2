								...
								if(paired) {
									...
									ret = sd.extendSeedsPairedNoEE(
										...
										*rds[mate ^ 1], // mate to align as opp.
										...
										!deterministicSeeds,  // use random subsampling?
										ca,             // seed alignment cache
										rnd,            // pseudo-random source
										wlm,            // group walk left metrics
										swmSeed,        // DP metrics, seed extend
...
// create the right kind of worker thread, based on cmdline parameters
template<typename T>
inline T* new_worker_thread(void* args)
{
	if(bowtie2p5) {
...
static void steal_threads(int pid, int orig_nthreads, EList<int>& tids, EList<T*>& threads)
{
	...
	if(in_use < ncpu) {
		...
		threads.push_back(new_worker_thread<T>((void*)&tids.back()));
	...
	OutFileBuf *metricsOfb)
{
	...
	all_threads_done = 0;
	{
		...
		for(int i = 0; i < nthreads; i++) {
			...
			threads.push_back(new_worker_thread<std::thread>((void*)&tps[i]));
			threads[i]->detach();
			SLEEP(10);
		}
